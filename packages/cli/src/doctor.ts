...
async function readPackageJson(projectDirectory: string) {
  try {
    return JSON.parse(await readFile(resolve(projectDirectory, "package.json"), "utf8")) as PackageJson;
  } catch {
    return undefined;
  }
}
...
export async function doctor(projectDirectory: string) {
  ...
  if (!(await hasConfig(projectDirectory))) {
    ...
    checks.push({ detail: `${config.aliases.ui} → ${uiDirectory}`, name: "UI 별칭", status: "pass" });
  }
  ...
  checks.push(
    hasDependency(packageJson, "@stylexjs/unplugin")
      ? { detail: "@stylexjs/unplugin이 설치되어 있습니다.", name: "StyleX 빌드 플러그인", status: "pass" }
      : {
